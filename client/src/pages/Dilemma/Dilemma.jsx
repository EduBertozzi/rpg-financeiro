    ...
    <div className="min-h-screen bg-darker">
      ...
      <div className="flex items-center justify-center" style={{ minHeight: 'calc(100vh - 65px)' }}>
        <div className="text-center">
          ...
          <button onClick={handleBack} className="px-6 py-3 bg-primary text-white rounded-lg">Voltar ao Mapa</button>
        </div>
      </div>
    </div>
  )
          ...
          <div className="bg-card border border-yellow-700 rounded-2xl p-8">
            {!result ? (
                  ...
                  {dilemma.options.map((option, index) => (
                    <button key={index} onClick={() => handleChoose(index)} disabled={choosing}
                      ...
                      <div className="flex items-start gap-3">
                        <span className="text-yellow-400 font-bold text-lg">{option.label}</span>
              ...
              <div className="text-center space-y-6">
                ...
                <button onClick={handleBack} className="px-8 py-3 bg-primary hover:bg-blue-600 text-white font-bold rounded-xl transition-colors">
                  Voltar ao Mapa
                </button>
              </div>
            )}
